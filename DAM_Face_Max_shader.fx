...
outlineOutput outline_VS(appdata IN)
{
    ...
    //normal ->观察空间
    float3 nDirVS = mul(IN.Normal,ViewXf);
    //view -> hclip -> NDC
    float4 nDirVS4 = float4(nDirVS,1);
    float4 nDirNDC = normalize(mul(nDirVS4,ProjectionXf)) * OUT.posCS.w ;

   
    OUT.posCS.xy = OUT.posCS.xy + nDirNDC.xy *  outlineSize * 0.001 *IN.Colour.b;
